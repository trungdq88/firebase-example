...
Rx.Observable.fromEvent(btnFacebook, 'click').subscribe(() => auth.signInWithRedirect(new firebase.auth.FacebookAuthProvider()));
Rx.Observable.fromEvent(btnLogout, 'click').subscribe(() => auth.signOut());
...
// Handle events
Rx.Observable.fromEvent(range, 'touchstart').startWith(sampleTouch).flatMap(() => Rx.Observable.fromEvent(range, 'touchmove').startWith(sampleTouch).takeUntil(Rx.Observable.fromEvent(range, 'touchend')).concat(Rx.Observable.just(sampleTouch))).map(e => {
  e.preventDefault();
  return {
...
}).debounce(10).filter(() => auth.currentUser !== null).subscribe(value => {
